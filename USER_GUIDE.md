...
> 🌐 在线版直接访问 **http://39.106.56.174:8080**

首次使用需要**注册账号**，点击页面上的「注册」链接，填写用户名和密码即可。
...
系统首页，全新 Landing Page 设计：

- **Hero 区域**：渐变色标题 + 平台简介 + 两个 CTA 按钮（"开始回测" / "AI 生成策略"）
- **功能卡片矩阵**：6 张卡片展示全部核心功能，每张卡片可点击直达对应页面
- **底部信息栏**：数据源徽章（AKShare / Local / Tushare）、市场覆盖说明、版本号

---

### 2.2 智能选股 `#/screener` ⭐ 新功能

**功能**：选股器海选 + 策略买点确认的一站式筛选流程。

**工作流程**：
1. 选择选股器（如 `momentum_screener`）和策略（如 `ma_cross`）
2. 设定筛选日期和 Top-N 数量
3. 点击「开始选股」
4. 系统先由选股器从全市场筛选候选标的
5. 再用策略对每个候选确认是否有买点信号
6. 返回带评分的排序结果

**结果展示**：
- **统计卡片**：扫描总数、候选数、有买点数、命中率
- **筛选切换**：「仅看有买点」复选框快速过滤
- **排序表格**：评分条（颜色渐变）+ 因子明细 + 买点信号 + 关键指标
- **点击行**：跳转单股分析，自动带入股票代码和策略
...
**结果展示** ⭐ 增强：
- **权益曲线图表**：ECharts 交互式曲线，含回撤叠加和买卖标记
- **交易明细表**：完整买卖记录，含触发原因标签（策略信号 / 出场规则）
- **统计卡片**：初始资金 → 最终权益、总收益率、胜率、夏普比率
- **指标面板**：所有回测指标的详细数值

---

### 2.8 AI 策略 `#/ai-strategy`
...
智能选股是一个**选股器海选 + 策略买点确认**的二阶段筛选流程：

1. **海选阶段**：选股器（Screener）从全市场按多因子评分筛选出 Top-N 候选标的
2. **确认阶段**：对每个候选，运行指定策略检测是否存在买点信号
3. **结果排序**：有买点的股票优先，同分按评分降序

### 6.2 如何使用

1. 点击左侧「选股」
2. 选择选股器：`momentum_screener`（7 因子动量）/ `hot_money_screener`（游资打板）
3. 选择验证策略：如 `ma_cross`
4. 选择日期（默认今天）
5. 设置返回数量（默认 Top 20）
6. 可选：展开参数面板调整选股器和策略参数
7. 点击「开始选股」
...
| 有买点 | 策略确认存在买点信号的股票数 |
| 命中率 | 有买点 / 扫描股票 × 100% |
| 评分条 | 绿色 ≥0.8 / 橙色 ≥0.6 / 灰色 <0.6 |
| 因子明细 | 鼠标悬停查看每只股票的因子级评分 |
| 关键指标 | 收盘价、成交量、成交额、均线 |

### 6.4 典型工作流

```
选股器扫描 → 发现候选股票 → 策略验证买点 → 点击"查看" → 跳转单股分析做深度回测
```
...
BacktestLab 使用 JWT（JSON Web Token）进行用户认证。

**注册**：
1. 首次访问时，点击「注册」链接
2. 填写用户名（至少 2 个字符）和密码（至少 4 个字符）
3. 注册成功后自动登录

**登录**：
1. 已有账号直接输入用户名和密码
2. 登录成功后获得 7 天有效的 Token
3. Token 过期后需重新登录

### 11.2 安全机制

- **密码哈希**：PBKDF2-HMAC-SHA256，60 万次迭代 + 随机盐
- **Token 签名**：HS256 算法，服务端密钥签名
- **自动过期检测**：Token 过期时自动跳转登录页，不会丢失数据
- **原子写入**：用户数据 JSON 文件使用临时文件 + `os.replace` 保证写入安全

### 11.3 用户状态

登录后在左下角可看到当前用户名和头像。点击「退出」按钮即可登出。
