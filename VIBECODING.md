...
8. [阶段六：体验升级](#8-阶段六体验升级-2026-06-30)
...
```
2026-06-19 ━━━ 2026-06-21 ━━━ 2026-06-27 ━━━ 2026-06-28 ━━━ 2026-06-30
    │              │              │              │              │
  架构奠基      图表+选股     组合+AI双翼   AI策略+整合    体验升级 + 收尾打磨
28 commits    20 commits    21 commits    18 commits    30 commits  46 commits
```
...
| ---------------- | ------------------------------------- |
| 总开发天数       | **13 天**                             |
| 总 Git 提交      | **165+ commits**                      |
| Python 源文件    | **58 个**（`autotrade/`）             |
| React/TS 源文件  | **30 个**（`web/src/`）               |
| 单元/集成测试    | **25+ 测试文件**                      |
| 设计规约（Spec） | **11 份**                             |
| 实现计划（Plan） | **11 份**                             |
| 架构决策（ADR）  | **4 份**                              |
...
> **目标**：鉴权系统、智能选股、组合回测可视化、性能优化  
> **产出**：JWT 鉴权 + Screener 页面 + 组合权益图 + 代码分割 + 46 commits

这是将系统从"能用"推向"好用"的关键阶段。

### 8.5.1 JWT 鉴权系统

| 产出                             | 说明                                                             |
| -------------------------------- | ---------------------------------------------------------------- |
| `autotrade/auth/auth.py`         | PBKDF2-HMAC-SHA256 密码哈希（60 万次迭代）、HS256 JWT 签发/验证  |
| `autotrade/auth/store.py`        | 线程安全 JSON 用户存储，原子写入防数据丢失                       |
| `autotrade/auth/routes.py`       | `/api/auth/register`、`/api/auth/login`、`/api/auth/me` 三个端点 |
| `autotrade/auth/dependencies.py` | FastAPI 依赖注入，Bearer Token 提取 + JWT 验证                   |
| `web/src/hooks/useAuth.tsx`      | React Auth Context：登录/注册/登出/Token 管理/过期检测           |
| 前端页面                         | `Login.tsx` + `Register.tsx` + `ProtectedRoute` 路由守卫         |

### 8.5.2 智能选股页面

| 产出                                                       | 说明                                      |
| ---------------------------------------------------------- | ----------------------------------------- |
| `autotrade/core/engine.py` → `run_screen()`                | 选股器海选 + 策略买点确认的二阶段筛选引擎 |
| `autotrade/core/interfaces.py` → `Screener.explain()`      | 新增因子评分明细接口                      |
| `autotrade/core/models.py` → `ScreenItem` / `ScreenResult` | 选股结果数据模型                          |
| `/api/screen` + `/api/screeners`                           | 选股 API 端点                             |
| `web/src/pages/Screener.tsx`                               | 新页面：表单 → 统计卡片 → 可排序表格      |
| `web/src/components/ScreenerParamsEditor.tsx`              | 选股器参数动态编辑器                      |

### 8.5.3 组合回测可视化增强

- **权益曲线图表**：ECharts 交互式曲线，含回撤叠加区域和买卖标记散点
- **交易明细表**：完整买卖记录，trigger 字段区分策略信号/出场规则
- **指标面板**：所有回测指标的详细数值展示
- **输出目录**：结果文件（equity_curve.csv / trades.csv / summary.json）保存路径提示

### 8.5.4 Dashboard 重新设计

- 从简单的状态面板改造为功能展示 Landing Page
- Hero 区域 + 6 张功能卡片（每张可点击直达）
- 底部数据源徽章 + 市场覆盖信息
...
| 优化项     | 实现                                                                         |
| ---------- | ---------------------------------------------------------------------------- |
| 代码分割   | 所有页面 `React.lazy()` + `Suspense`，首屏体积减半                           |
| GZip 压缩  | FastAPI GZipMiddleware，文本资源压缩率 70%+                                  |
| 缓存 Hooks | `useCachedStrategies`、`useCachedGroups`、`useStockSearch`                   |
| 搜索防抖   | 股票搜索 debounce 500ms，减少无效 API 请求                                   |
| API 前缀   | 所有端点统一 `/api` 前缀，消除前后端路由冲突                                 |
| SPA 回退   | 基于 `Sec-Fetch-Dest` 头判断，浏览器刷新 `/groups` 等路径正确返回 index.html |

### 8.5.6 新工具脚本

| 脚本                             | 用途                                                              |
| -------------------------------- | ----------------------------------------------------------------- |
| `scripts/run_ai_backtest.py`     | AI 增强组合回测：LocalAIAnalyzer（本地数据 + DeepSeek）+ LLMCache |
| `scripts/run_turtle_backtest.py` | 海龟策略批量回测：支持全市场 / 分组 / 单股票，长仓可选            |
...
| Python AI 模块   | 7        | ~1,800 行          |
| Python Auth 模块 | 4        | ~400 行            |
| Python API       | 2        | ~1,600 行          |
| Python 测试      | 25+      | ~6,000 行          |
| React/TS 前端    | 30       | ~6,500 行          |
| YAML 配置        | 30       | ~1,200 行          |
| 文档（md）       | 28+      | ~60,000 词         |
| **总计**         | **170+** | **~28,000 行代码** |
