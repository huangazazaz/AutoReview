 ...
 * 设计: AutoTrade v3 — 增强卡片 / 渐变强调 / 图表区美学优化
 */
(function () {
    'use strict';
            ...
            <div class="page-hero">
                <div class="page-header" style="margin-bottom:0;">
                    <h1 class="page-title" style="display:flex;align-items:center;gap:var(--space-3);">
                        <span style="display:flex;align-items:center;justify-content:center;width:40px;height:40px;border-radius:var(--radius);background:var(--gradient-accent);color:#fff;">${Search({ size: 22, class: '' })}</span>
                        单股分析
                    </h1>
                    <p class="page-subtitle" style="margin-top:4px;">选择股票和策略，运行回测查看详细指标与交易记录</p>
                </div>
            </div>
    ...
    function renderResults(data, bars = []) {
        ...
        const stockDisplay = data.stock_name
            ? `${escapeHtml(data.symbol)} <span style="color:var(--text-secondary);font-weight:400;font-size:16px;">${escapeHtml(data.stock_name)}</span>`
            : escapeHtml(data.symbol);

        const trendIcon = isPositive ? TrendingUp({ size: 20 }) : TrendingDown({ size: 20 });

        document.getElementById('analyze-results').innerHTML = `
            <!-- 股票标题 + 趋势指示 -->
            <div class="card card-gradient" style="margin-top:var(--space-5);padding:var(--space-4) var(--space-5);">
                <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:var(--space-3);">
                    <div style="display:flex;align-items:center;gap:var(--space-3);">
                        <span style="display:flex;align-items:center;justify-content:center;width:36px;height:36px;border-radius:50%;background:${isPositive ? 'var(--buy-bg)' : 'var(--sell-bg)'};color:${isPositive ? 'var(--buy)' : 'var(--sell)'};">${trendIcon}</span>
                    ...
                    <div style="display:flex;align-items:center;gap:var(--space-2);">
                        <span class="trend-indicator ${isPositive ? 'trend-up' : 'trend-down'}">
                            <span class="trend-arrow">${isPositive ? '▲' : '▼'}</span> ${formatPct(ret)}
                        </span>
                        <span style="font-size:12px;color:var(--text-muted);">回测结果</span>
                    </div>
                </div>
            </div>
            ...
            <div class="stats-grid" style="margin-top:var(--space-4);" role="status" aria-label="回测核心指标">
                <div class="stat-card" style="border-top:2px solid ${isPositive ? 'var(--buy)' : 'var(--sell)'};">
                ...
                <div class="stat-card" style="border-top:2px solid var(--sell);">
                    <div class="stat-label">📉 最大回撤</div>
                    <div class="stat-value stat-negative">${formatPct(m.max_drawdown_pct)}</div>
                    <div class="stat-sub">从最高点计算的最大跌幅</div>
                </div>
            ...
            <div class="stats-grid">
                <div class="stat-card" style="border-top:2px solid var(--primary);">
                    ...
                    <div class="stat-sub">
                        买 <span style="color:var(--buy);font-weight:600;">${m.buy_trades || 0}</span> ·
                        卖 <span style="color:var(--sell);font-weight:600;">${m.sell_trades || 0}</span> ·
                        信号 <span style="color:var(--info);font-weight:600;">${data.signal_count || 0}</span>
                    </div>
                </div>
                <div class="stat-card" style="border-top:2px solid var(--buy);">
                ...
                <div class="stat-card" style="border-top:2px solid var(--warning);">
            ...
            <div class="card card-accent" style="border-top:2px solid rgba(139,92,246,0.3);">
                <div class="card-header">
                    <span class="card-title" style="display:flex;align-items:center;gap:var(--space-2);">
                        <span style="color:var(--accent);display:flex;">${ChartIcon({ size: 18 })}</span> 净值曲线
                    </span>
            ...
            <!-- 交易记录 -->
            <div class="card card-accent" style="border-top:2px solid rgba(59,130,246,0.3);">
                <div class="card-header">
                    <span class="card-title" style="display:flex;align-items:center;gap:var(--space-2);">
                        <span style="color:var(--info);">📜</span> 交易记录
                    </span>
                    <span style="font-size:12px;color:var(--text-muted);">共 ${trades.length} 笔</span>
                </div>
                <div class="card-body" style="padding:0;">
                    ${trades.length === 0 ? `
                    <div class="empty-state-enhanced">
                        <div class="empty-icon-bg">${File({ size: 32 })}</div>
                        <div class="empty-title">暂无交易记录</div>
                        <div class="empty-desc">该策略在此时间段内未产生任何交易信号</div>
                    </div>` : `
                        ...
                        <table aria-label="交易记录明细">
                            ...
                            <tbody>
                                ${trades.map((t, i) => {
                                    ...
                                    const rowBg = isBuy ? 'rgba(239,68,68,0.02)' : 'rgba(34,197,94,0.02)';
                                    return `
                                    <tr title="${escapeHtml(reason)}" style="background:${rowBg};">
                                        ...
                                        <td style="text-align:right; color:${isBuy ? 'var(--buy)' : 'var(--sell)'};font-weight:600;">${formatAmount(amount)}</td>
    ...
    // renderEquityChart 和 renderAnalyzeParams 保持不变
        ...
        const option = {
            ...
            yAxis: [
                ...
                },
                {
                    ...
                    scale: true,
                    splitLine: { show: false },
                    axisLabel: { color: '#F59E0B', fontSize: 10, formatter: v => v.toFixed(2) },
                    name: '股价 (元)',
            ...
            series: [
                {
                    name: '基准', type: 'line', data: baseline,
                    xAxisIndex: 0, yAxisIndex: 0,
                    lineStyle: { color: '#64748B', type: 'dashed', width: 1.5, dashOffset: 2 },
                    itemStyle: { color: '#64748B' }, symbol: 'none', z: 1,
                },
                {
                    name: '净值', type: 'line', data: equities,
                    xAxisIndex: 0, yAxisIndex: 0,
                    ...
                    markPoint: equities.length > 5 ? {
                        data: [
                            { type: 'max', name: '最高', symbol: 'pin', symbolSize: 40,
                              ...
                              label: { color: '#F59E0B', fontSize: 11, fontWeight: 600, formatter: p => formatAmount(p.value) } },
                            { type: 'min', name: '最低', symbol: 'pin', symbolSize: 40,
                              itemStyle: { color: '#EF4444' },
                              label: { color: '#EF4444', fontSize: 11, fontWeight: 600, formatter: p => formatAmount(p.value) } },
                        ],
                    } : undefined,
                    z: 2,
                },
                {
                    name: '回撤', type: 'line', data: drawdowns,
                ...
                ...(hasClosePrice ? [{
                    ...
                    itemStyle: { color: '#F59E0B' }, symbol: 'none', z: 1,
                }] : []),
                ...(hasTradeMarkers ? [{
                    ...
                    itemStyle: { color: '#EF4444' },
                    label: { show: true, position: 'top', color: '#EF4444', fontSize: 10, fontWeight: 600, formatter: 'B' },
                    z: 10,
                }, {
                    ...
                    label: { show: true, position: 'bottom', color: '#22C55E', fontSize: 10, fontWeight: 600, formatter: 'S' },
                    z: 10,
                }] : []),
            ],
        };
