...
export default function StrategyParamsEditor({ strategy, editParams, onChange }: Props) {
  const [expanded, setExpanded] = useState(false)

  if (!strategy?.param_schema || Object.keys(strategy.param_schema).length === 0) {
    return null
  }
  ...
  return (
    <div className="params-section" style={{ marginTop: 16 }}>
      ...
      >
        <span className="toggle-icon">{expanded ? '▼' : '▶'}</span> 策略参数
        ...
        <div className="params-body">
            ...
            {simpleEntries.map(([key, info]) => {
              ...
              return (
                <div key={key} className="param-item">
                  <label className="param-label" title={`${key}${info.default !== undefined ? ' · 默认: ' + JSON.stringify(info.default) : ''}`}>
                    {key}
                  </label>
                  <input
                    type={inputType}
          ...
          {complexEntries.map(([key, info]) => (
            <div key={key} className="param-item param-item-wide">
              <label className="param-label">{key} <span className="param-type-tag">{info.type}</span></label>
              <textarea
                className="form-input"
                rows={2}
                value={editParams[key] ?? ''}
